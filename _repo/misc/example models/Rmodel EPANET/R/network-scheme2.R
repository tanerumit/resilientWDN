...
pNetwork <- graph_from_edgelist(as.matrix(edgelist[,c('from','to')])); 
vertex_attr(pNetwork)$color <- sapply(nodelist$type, function(x) ifelse(x == "s", "steelblue", "orange"))
vertex_attr(pNetwork)$label <- nodelist$label

plot(pNetwork, vertex.color = vertex_attr(pNetwork)$color,  vertex.label = vertex_attr(pNetwork)$label)
