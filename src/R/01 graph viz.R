...
ggsave(filename = paste0(plot_dir, "friesland_nodes.png"), width = pwidth, height = pheight)
...
for (x in 1:designNum) {
  ...
  ggsave(filename = paste0(plot_dir, "friesland_design",x ,".png"), width = pwidth, height = pheight)
...
ggsave("./graphics/demand/demand_viz.png", width = 7, height = 5)


  range(nodesDF2$discharge)
  geom_edges(aes(x=x, y=y, xend=xend, yend=yend, color = colid), data=pipesDF, size=1) +
