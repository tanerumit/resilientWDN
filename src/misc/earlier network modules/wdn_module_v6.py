...
class Pipe:
    ...
    def __init__(self, id, start, end, length, diameter, disuse, q_max=np.inf, **kwargs):
        self.id = id
        self.start = start
        self.end = end
        self.length = length
...
#    print(calculate_network(pd.read_csv('network2_nodes.csv'), pd.read_csv('network2_design1.csv'), 1.0, 1))
