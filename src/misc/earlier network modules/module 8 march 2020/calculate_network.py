...
class Pipe:
    ...
    def __init__(self, id, start, end, length, diameter, disuse, flowdir, q_max=np.inf, **kwargs):
        self.id = id
        self.start = start
        self.end = end
        self.length = length
        ...
        if head_loss_option >= HeadLossOption.LINEAR:
            ...
            if head_loss_option == HeadLossOption.LINEAR:
                ...
                for a, b in zip(*_get_head_loss_coeffs(p.length, p.diameter / 1000)):
                    equations.append((p.head_loss - (a * p.q_start + b - (1 - p.flow_dir_sym) * max_discharge), 0.0, np.inf))
                    equations.append((-p.head_loss - (a * -p.q_start + b - p.flow_dir_sym * max_discharge), 0.0, np.inf))
