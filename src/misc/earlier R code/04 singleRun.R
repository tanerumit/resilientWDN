...
friesland_pipes  <- read.csv("./data/v7/pipes_d1.csv")
...
# penalty_order: penalty weight assigned to deficits 
# (penalty order can be used to make sure shortages are spread out by setting it 
# to 2.0 - only works for head_loss_option 1 or 2)
# head_loss_option: 1 = discharge only, 2 = linear head loss equation, 
# 3 = mixed integer problem with non-linear (more accurate) head loss equation.

# Calculate network
out1 <- calculate_network(node_df, pipe_df, 1.0, 1)
out2 <- calculate_network(node_df, pipe_df, 2.0, 2)
out3 <- calculate_network(node_df, pipe_df, 1.0, 4)
