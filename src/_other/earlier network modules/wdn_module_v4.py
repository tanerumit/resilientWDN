...
def calculate_network(nodes: pd.DataFrame,
    ...
    if head_loss_option <= HeadLossOption.LINEAR:
        ...
        if False: #penalty_order == 1.0:
            solver = ca.qpsol('qp', 'clp', qp, {'clp': {'slog': 0}})
        else:
        ...
        solver = ca.qpsol('qp', 'cbc', qp, {"discrete": discrete, 'cbc': {'slog': 0}})
...
#node_info = pd.read_csv("nodes.csv", sep='\t')
#pipe_info = pd.read_csv("pipes.csv", sep='\t')

#calculate_network(nodes = node_info, pipes = pipe_info, penalty_order = 1.0, head_loss_option = 3)