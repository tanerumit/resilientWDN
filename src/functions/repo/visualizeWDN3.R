  ...
  
{
  ...
  pipesDF <- pipes.data %>%
    ...
    left_join(y = (nodes.data %>% select(end = id, lon_end = lon, lat_end = lat)), by = "end") %>%
    mutate(id = as.factor(id))
  
  # Edge parameters
  edge_color <- rep("gray70", nrow(pipesDF))
  node_color <- rep("gray50", nrow(nodesDF))
  node_size  <- rep(1, nrow(nodesDF))
  node_fill  <- rep("black", nrow(nodesDF))
  
  # Draw base graph
  p <- ggmap(background.map, extend = "device")
  
  # Draw pipe network
  p <- p + geom_edges(aes(x = lon, y = lat, xend = lon_end, yend = lat_end, 
                 color = id), size = 1, data = pipesDF)  
    
  # Draw demand/supply nodes
  p <- p + geom_nodes(
      mapping = aes(x = lon, y = lat, fill = id),
  ...
  p <- p + 
    scale_color_manual(limits = pipesDF$id, values = edge_color) +
    scale_fill_manual(limits = nodesDF$id, values = node_fill) +
    geom_text(aes(x = lon, y = lat, label = label), data = nodesDF) +
    labs(x = xlab, y = ylab, fill = "", size = "", color = "") +
